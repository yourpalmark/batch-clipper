...
const RATE_LIMIT_DELAY_MS = 800;   // delay between API calls to avoid 429s
  ...
  while (!isCancelled()) {
    ...
    if (resp.status === 429) {
      for (let attempt = 1; attempt <= 4; attempt++) {
        ...
        console.warn(`[BatchClipper] 429 at depth=${currentDepth} pageId=${pageId} — waiting ${wait}ms (attempt ${attempt}/4)`);
        await _sleep(wait);
