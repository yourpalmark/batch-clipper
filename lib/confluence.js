...
const RATE_LIMIT_DELAY_MS = 300;   // delay between API calls to avoid 429s
const RATE_LIMIT_RETRY_MS = 5000;  // wait time on 429 before retrying once
...
async function fetchChildPages(baseUrl, pageId, maxDepth, currentDepth = 0, isCancelled = () => false, onProgress = null, _counter = null) {
  ...
  while (!isCancelled()) {
    ...
    if (resp.status === 429) {
      console.warn(`[BatchClipper] 429 rate limit at depth=${currentDepth} pageId=${pageId} — waiting ${RATE_LIMIT_RETRY_MS}ms then retrying`);
      await _sleep(RATE_LIMIT_RETRY_MS);
      resp = await fetch(url, { credentials: 'include' });
    }
    if (!resp.ok) {
      console.warn(`[BatchClipper] fetchChildPages depth=${currentDepth} pageId=${pageId} HTTP ${resp.status} — stopping recursion here`);
      break;
    }
    ...
    for (const child of data.results) {
      ...
      console.log(`[BatchClipper] found depth=${currentDepth} id=${child.id} title="${child.title}"`);
