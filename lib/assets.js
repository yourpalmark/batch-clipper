...
function rewriteMarkdownUrls(markdown, rewriteMap) {
  ...
  const rewriteSegment = (text) => {
    ...
    // URL may contain parentheses (e.g. filenames like "image (1).png") so we can't
    // use [^)]+ — instead match everything up to the last ) on the line using .*?
    // with a possessive-style approach: match any char including ) as long as the
    // overall pattern can close. We do this by allowing balanced parens one level deep.
