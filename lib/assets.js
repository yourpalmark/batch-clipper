...
function buildRewriteMap(assets, pageTitle, assetsPrefix) {
  ...
  for (const asset of assets) {
    ...
    // Markdown image links need percent-encoding; wikilinks need literal text.
    // Store both so rewriteMarkdownUrls can pick the right one.
    const markdownPath = `${assetsPrefix}/${encodeFn(pageTitle)}/${encodeFn(asset.filename)}`;
    const wikilinkPath = `${assetsPrefix}/${pageTitle}/${asset.filename}`;
    map.set(decoded, { markdownPath, wikilinkPath });
  }
  return map;
}
...
function rewriteMarkdownUrls(markdown, rewriteMap) {
  ...
  let result = markdown.replace(/(!?)\[([^\]]*)\]\(([^)]+)\)/g, (match, bang, alt, url) => {
    const filename = filenameFromUrl(url.trim());
    if (filename && rewriteMap.has(filename)) {
      const { markdownPath, wikilinkPath } = rewriteMap.get(filename);
      if (isImageFilename(filename)) {
        // Image embed: Obsidian ![[path]] wikilink (literal path, no percent-encoding)
        return `![[${wikilinkPath}]]`;
      } else {
        // Non-image attachment: ![[path]] wikilink with literal path so Obsidian embeds/opens as file
        return `![[${wikilinkPath}]]`;
      }
    }
    return match;
