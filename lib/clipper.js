...
function preprocessHtml(doc, baseOrigin = '') {
  ...
  try {
    for (const macro of doc.querySelectorAll('[data-macro-name="drawio"], [data-macro-name="drawio-sketch"]')) {
      ...
      // If content.js already replaced the SVG with an img (active tab / liveHtml),
      // the container will have an img and no svg — skip to avoid duplicates.
      if (container && container.querySelector('img') && !container.querySelector('svg')) continue;
      ...
      if (svg) {
        // SVG present (live DOM already rendered) — replace it
        svg.replaceWith(img);
      } else {
        // No SVG — insert img after the macro div so it survives macro removal
