...
function preprocessHtml(doc, baseOrigin = '') {
  ...
  // Absolutize relative Confluence links so they open in the browser rather than
  // creating ghost Obsidian files. Only touches root-relative paths (/display/...,
  // /pages/..., /wiki/...) — leaves anchors, mailto:, external URLs, and empties alone.
  if (baseOrigin) {
    try {
      for (const a of doc.querySelectorAll('a[href]')) {
        ...
        if (href.startsWith('/')) {
          try { a.setAttribute('href', new URL(href, baseOrigin).href); } catch { /* ignore */ }
        }
      }
    } catch { /* ignore */ }
  }

  // Confluence user profile links (/display/~username) — strip link, keep text.
  // These may still have relative or absolute forms; handle after absolutizing above.
  try {
 ...
 *   <vaultDir>/<clipSubfolder>/[...hierarchy]/<Title>.md
 *   <vaultDir>/<clipSubfolder>/<assetSubfolder>/[...hierarchy]/<Title>/<image.png>  (when downloadAssets=true)
 *
 * Assets are always rooted at <clipSubfolder>/<assetSubfolder>/ and mirror the page hierarchy,
 * so the entire tree has one flat assets folder at the top level of the clip subfolder.
 *
 * @param {object} params
 ...
 * @param {string[]} [params.assetPathSegments=[]] - path segments inserted between assetSubfolder and
 *   page title folder (mirrors the page's position in the hierarchy, e.g. ["Parent", "Child"])
...
async function clipPage({
  ...
  try {
    ...
    if (confluenceBaseUrl) {
      // Confluence embeds the space name in doc.title ("Page Title - Space Name").
      // Use ajs-page-title meta (page title only) when available.
      try {
        const ajsTitle = doc.querySelector('meta[name="ajs-page-title"]')?.getAttribute('content')?.trim();
        if (ajsTitle) title = ajsTitle;
      } catch { /* ignore */ }
    ...
    // Assets are stored at: <clipSubfolder>/<assetSubfolder>/[...assetPathSegments]/<Title>/img.png
    // The wikilink prefix from within the .md file must traverse back up to <clipSubfolder> level
    // then into <assetSubfolder>/[...assetPathSegments].
    // Since the .md may be nested (e.g. raw/Parent/Child/Page.md), we need vault-root-relative
    // wikilinks. Obsidian resolves [[path/to/img.png]] from vault root when given a full path.
    if (downloadAssets && assets.length > 0) {
      const assetPrefixParts = [assetSubfolder, ...assetPathSegments];
      const assetsPrefix = assetPrefixParts.join('/');
      const rewriteMap = buildRewriteMap(assets, safeTitle, assetsPrefix);
    ...
    if (downloadAssets && assets.length > 0) {
      ...
      // Assets are rooted at <clipSubfolder>/<assetSubfolder>/[...assetPathSegments]/<Title>/
      // Build from vaultDir so asset root is always sibling of the page hierarchy, not nested inside it.
      const clipRootDir = await getOrCreateDir(vaultDir, clipSubfolderSegments ? [clipSubfolderSegments[0]] : [clipSubfolder]);
      const assetRootDir = await getOrCreateDir(clipRootDir, [assetSubfolder, ...assetPathSegments]);
      let assetSubDir;
      try {
