...
function preprocessHtml(doc, baseOrigin = '') {
  ...
  // Two cases:
  // 1. Heading whose ONLY content is an image (e.g. a diagram used as a section banner):
  //    replace the entire heading with a <p> containing the image — Turndown then
  //    produces a clean standalone ![]() instead of ## ![](). The heading title is lost
  //    but it was just the image anyway.
  // 2. Heading that has text + an image: move the image(s) to just after the heading
  //    so they survive defuddle's heading transforms without corrupting the heading text.
  try {
    for (const heading of doc.querySelectorAll('h1, h2, h3, h4, h5, h6')) {
      ...
      // Check if the heading's text content (excluding the images) is empty/whitespace
      const clone = heading.cloneNode(true);
      for (const img of clone.querySelectorAll('img')) img.remove();
      const textOnly = clone.textContent.trim();

      if (!textOnly) {
        // Image-only heading: replace the whole heading with <p> wrappers for each image.
        // Use the actual img nodes (not clones) so they are moved out of the heading.
        const fragment = doc.createDocumentFragment();
        for (const img of Array.from(imgs)) {
          const p = doc.createElement('p');
          p.appendChild(img); // moves the node, detaching it from heading
          fragment.appendChild(p);
        }
        heading.replaceWith(fragment);
      } else {
        // Heading has text: move images out to after the heading
...
async function clipPage({
  ...
  try {
    ...
    // Fix image-only headings that survived DOM preprocessing: '## ![alt](url)' should be
    // a standalone image, not a heading. Turndown emits this when an <h*> contains only an
    // <img>. Strip the heading prefix so Asset Swapper can match and replace the URL.
    markdown = markdown.replace(/^(#{1,6})\s+(!\[[^\]]*\]\([^)]*\))\s*$/gm, '$2');
